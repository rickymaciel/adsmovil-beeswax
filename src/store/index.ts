...
import {
	ResultPaginate as UserResultPaginate,
	UserDataCreate,
	User
...
import {
	...
	ModifierDataCreate,
	ModifierDataUpdate,
	ResultPaginate as ModifierResultPaginate
	...
	modules: {
		...
		advertiser: {
			...
			mutations: {
				...
				SET_ADVERTISER(
					state,
					_advertiser: Advertiser = {} as Advertiser
				) {
					state.advertiser = _advertiser;
				},
				SET_ADVERTISERS(
					state,
					_advertisers: Advertiser[] = [] as Advertiser[]
				) {
			...
			actions: {
				async paginated({ commit }, params) {
					try {
						const response = await AdvertiserService.paginated(
							params
						);
				...
				async create({}, advertiser: AdvertiserDataCreate) {
					try {
						const response = await AdvertiserService.create(
							advertiser
						);
						notificationService.notifySuccess(this, {
							to: "AdvertisersIndex"
				...
				async all({ commit }) {
					try {
						const response = await AdvertiserService.all(
							undefined,
							{
								order: "asc",
								sort: "name"
		...
		custom_list: {
			...
			mutations: {
				...
				SET_CUSTOM_LIST(
					state,
					_custom_list: CustomList = {} as CustomList
				) {
					state.custom_list = _custom_list;
				},
				SET_BIDDING_SHADING_LIST(
					state,
					_bidding_shadings_list: CustomList = {} as CustomList
				) {
				...
				SET_CREATIVE_WEIGHTING_METHODS(
					state,
					_creative_weighting_methods: [] = []
				) {
			...
			actions: {
				async paginated({ commit }, params) {
					try {
						const response = await CustomListService.paginated(
							params
						);
						commit("SET_RESULT_PAGINATED", response);
						return await Promise.resolve(response);
				...
				async create({ commit }, customList: CustomListDataCreate) {
					try {
						const response = await CustomListService.create(
							customList
						);
				...
				async update({ commit }, updates) {
					try {
						const response = await CustomListService.update(
							updates
						);
						commit("SET_CUSTOM_LIST", response);
				...
				async uploadItems({}, params) {
					try {
						const response = await CustomListService.uploadItems(
							params.payload,
							params.function
						);
						notificationService.notifySuccess(
				...
				async getViewByTypeSelected({ commit }, type: Type) {
					try {
						const response = await CustomListService.getViewByTypeSelected(
							type
						);
		...
		list_item: {
			...
			actions: {
				async paginated({ commit }, params) {
					try {
						const response = await ListItemService.paginated(
							params
						);
						commit("SET_RESULT_PAGINATED", response);
						return await Promise.resolve(response);
				...
				async getAllByCustomId({ commit }, customList: any) {
					try {
						const entities = await ListItemService.getAllByCustomId(
							customList?.id
						);
		...
		user: {
			...
			mutations: {
				SET_RESULT_PAGINATED(
					state,
					_result_paginate: UserResultPaginate
				) {
					state.result_paginate = _result_paginate;
				},
			...
			actions: {
				...
				async update(
					{ commit },
					params: { user: UserDataUpdate; id: number }
				) {
					try {
				...
				async setAccount({}, account_id: number) {
					try {
						const response = await UserService.setAccount(
							account_id
						);
						return await Promise.resolve(response);
					} catch (error) {
		...
		campaign: {
			...
			mutations: {
				...
				SET_RESULT_LIST(
					state,
					_result_list: CampaingList[] = [] as CampaingList[]
				) {
			...
			actions: {
				...
				async paginated({ commit }, params) {
					try {
						const response = await CampaignService.paginated(
							params
						);
						commit("SET_RESULT_PAGINATED", response);
						return await Promise.resolve(response);
		...
		modifier: {
			namespaced: true,
			state: () => ({
				result_paginate: {} as ModifierResultPaginate,
			...
			mutations: {
				SET_RESULT_PAGINATED(
					state,
					_result_paginate: ModifierResultPaginate = {} as ModifierResultPaginate
				) {
					state.result_paginate = _result_paginate;
				},
				...
				SET_MODIFIERS(
					state,
					_modifiers: Modifier[] = [] as Modifier[]
				) {
			...
			getters: {
				result_paginate(state): ModifierResultPaginate {
					return state.result_paginate;
				}
			},
			actions: {
				async paginated({ commit }, params) {
					try {
						const response = await ModifierService.paginated(
							params?.paginated,
							params?.filters,
							params?.options
						);
				...
				async updateModifier(
					{ commit },
					params: { modifier: ModifierDataUpdate; id: number }
				) {
					try {
				...
				async list(
					...
					try {
						const response = await ModifierService.list(
							payload.filters,
							payload.options
						);
		...
		line_item: {
			...
			mutations: {
				...
				SET_LINE_ITEMS(
					state,
					_lineItems: LineItem[] = [] as LineItem[]
				) {
			...
			actions: {
				async create({  }, params: {data: LineItemDataCreate, continue: Boolean}) {
					try {
						const response = await LineItemService.create(params.data);
						console.log('@Action::create', {response, params});
				...
				async show({  }, id: number) {
					try {
						...
						if (response?.campaign?.id) {
							...
							console.log('@Action::show', {response, aux_campaign});
						}
						return await Promise.resolve(response);
					} catch (error) {
				...
				async update(
					{  },
					params: { data: LineItemDataCreate; id: number, continue: Boolean }
				) {
					try {
				...
				async paginated({ commit }, params) {
					try {
						const response = await LineItemService.paginated(
							params?.paginated,
							params?.filters,
							params?.options
						);
				...
				async list(
					...
					try {
						const response = await LineItemService.list(
							payload.filters,
							payload.options
						);
				...
				async getTargetingsByID({ }, id: number) {
					try {
						let response = await TargetingService.getTargetingsByID(id);
						console.log('getTargetingsByID', {id});
						return await Promise.resolve(response);
					} catch (error) {
						catchError(this, error, { to: "" });
						console.error(error);
		...
		creative: {
			...
			actions: {
				...
				async getCreativeTypeByTemplateId(
					...
					try {
						if (
							isEmpty(params.creativeTypes) &&
							params.creative_template_id < 1
						)
							return await Promise.resolve();
				...
				async creativeAssets({ commit }, params) {
					try {
						const response = await CreativeService.creativeAssets(
							params
						);
						return await Promise.resolve(response);
					} catch (error) {
				...
				async associateLineItemDelete({ commit }, id: number) {
					try {
						const response = await CreativeService.associateLineItemDelete(
							id
						);
						CreateNotification(this.dispatch, {
							type: MessageTypes.SUCCESS,
			...
			actions: {
				...
				async getAppNameByAtribute({ commit }, params) {
					try {
						...
						return await Promise.resolve(
							resolveListParams(response, "id", "app_name")
						);
					} catch (error) {
						catchError(this, error);
						return await Promise.reject(error);
				...
				async getSearchByTerm(
					...
					try {
						const response = await TargetingService.getSearchByTerm(
							params
						);
						return await Promise.resolve(
							resolveListParams(
				...
				async create({ commit }, targeting) {
					try {
						const response = await TargetingService.create(
							targeting
						);
						notificationService.notifySuccess(this, {
							to: ""
...
function catchError(store: any, error: any, params?: { to?: string }) {
