<template>
	<v-card flat justify="between" align="center" color="grey lighten-3 py-4">
		<v-form
			ref="form"
			justify="between"
			align="center"
				...
				<v-row v-if="resources.fields.line_pacing_id.show" dense>
					...
					>
						<CardTextField
							...
							:rules="[
								resources.fields.daily_budget_suggested.show &&
							...
							:persistent_hint="hasError('daily_budget')"
					...
					<v-col
						v-if="resources.fields.daily_budget_suggested.show"
						cols="12"
						sm="12"
						md="6"
						lg="4"
					>
						<CardTextField
							...
							:persistent_hint="calcSuggested === -1"
							:required="
								resources.fields.daily_budget_suggested.required
							"
							:disabled="
								resources.fields.daily_budget_suggested.disabled
							"
						></CardTextField>
					</v-col>
				</v-row>
				...
				>
					...
					<v-col cols="12" sm="12" md="4" lg="3">
						<CardAutocomplete
							...
							:reference="
								`unit_time_id_${frequency_cap.unit_time_id}`
							"
							item-text="value"
							item-value="id"
							label="Unit Time"
						...
						>
							<v-row align="center" justify="center">
								<v-btn
									...
									color="secondary"
									class="ma-2 px-8"
									@click="handleSubmit({ continue: false })"
								>
									{{ $t("save") }}
								</v-btn>
								<v-btn
									...
									color="secondary"
									class="ma-2 px-8"
									@click="handleSubmit({ continue: true })"
								>
									{{ $t("save-continue") }}
								</v-btn>
								<v-btn
		...
		async handleSubmit(params: { continue: Boolean } = { continue: true }) {
			try {
				...
				this.$emit("save", {
					...
					data: await this.prepareDataCreate()
				});
			} catch (error) {
		...
		async prepareDataCreate() {
			return {
				...
				cpm_bid: !!this.getLineItem?.cpm_bid
					? Number(this.getLineItem?.cpm_bid)
					: null,
	...
	watch: {
		async "getLineItem.campaign_id"(val, old) {
			if (isNull(val)) {
				...
				this.fetchResource("campaign_id", val);
			}
		},
