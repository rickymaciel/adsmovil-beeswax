...
import { AxiosPatch, AxiosPost } from '@/api/AxiosService'
import { VastWrapper, VastWrapperDataCreate, VastWrapperDataUpdate } from '@/interfaces/creativeVideoVastWrapper'
import {
...
export async function create (creative: VastWrapperDataCreate, token: string) {
  try {
...
export async function update (creative: VastWrapperDataUpdate, token: string) {
  try {
    const response = await AxiosPatch(ROUTES.CREATIVE_ROUTE + '/' + creative.id, creative, token)

    if (response.success) {
      const data = response.content

      const image = parseData(data)
...
function parseData (data): VastWrapper {
